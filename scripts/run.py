...
DOCKER_REPO = "fl4ming0/codeql:1.1"


def update_docker():
  PULL_CMD = "docker pull " + DOCKER_REPO
...
def parse_input(args):
  ...
  DOCKER_CMD = f'''docker run --rm --name codeql-docker -v "{SRC}:/opt/src" -v "{OUTPUT}:/opt/results" -e "LANGUAGE={LANG}" -e "FORMAT={FORMAT}" -e "QS={QS}" -e "THREADS={THREADS}" {DOCKER_REPO}'''
